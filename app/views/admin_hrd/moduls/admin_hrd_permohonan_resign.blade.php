...
<div id="div-controller" ng-controller="AdminHRDBacaPermohonanKaryawan">
	<div class="row broccoli-rowKepala">
		<h1> Permohonan <small> RESIGN </small> </h1>
		<br>
	...
	<div class="row broccoli-rowIsi">
		...
		<div class="col-md-12 broccoli-rowItable">
			<table class="table table-striped table-bordered table-hover" style="table-layout:fixed;">
				<tr ng-repeat="dp in data_permohonan | filter:query | orderBy:predicate:reverse">
					<td class="text-center" width="5%">@{{ $index+1 }}</td>
					<td class="text-center" width="15%">@{{ dp.nama }}</td>
					<td class="text-left" width="20%">@{{ dp.id_karyawan }}</td>
					<td class="text-left" width="19%">@{{ dp.jabatan }}</td>
					<td class="text-left" width="18%">@{{ dp.keterangan }}</td>
					<td class="text-center" width="12%">@{{ dp.status }} <!--<input type="checkbox"> </input>--></td>
